...
cleanup() {
  ...
  local failure_logs=()
  mapfile -t failure_logs < <(ls /tmp/arbeitszeitapp_failure_logs_*.tar.gz 2>/dev/null || true)
  if [[ ${#failure_logs[@]} -gt 0 ]]; then
    echo "=== PRESERVING FAILURE LOGS ==="
    for log_archive in "${failure_logs[@]}"; do
      local archive_name
      archive_name=$(basename "$log_archive")
  ...
  mapfile -t preserved_logs < <(ls ./arbeitszeitapp_failure_logs_*.tar.gz 2>/dev/null || true)
  ...
  *)
  ...
  for scenario in "${scenarios[@]}"; do
    ...
    *profiling_disabled*) profiling_enabled="false" ;;
    ...
    if (cd "$script_dir/../../docker-deployment"); then
